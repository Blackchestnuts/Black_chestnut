...
export async function buildMemoryPrompt(userId: string) {
  ...
  if (memories.length === 0 && !user?.occupation) {
    return `你是一个智能助手，拥有跨对话的记忆能力。你可以记住用户告诉你的信息，在后续对话中主动运用这些记忆来提供更个性化的服务。
...
当前你还没有关于这个用户的记忆，请在对话中自然地了解用户信息。`
  ...
  return `你是一个拥有跨对话记忆能力的智能助手。以下是你记住的关于用户的信息：
    ...
    for (const mem of memories) {
      ...
      if (existing) {
        await db.memory.update({
          ...
          data: { value: mem.value, updatedAt: new Date() },
        })
      } else {
